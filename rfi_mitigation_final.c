...
#define FATALF(fmt, ...) do { fprintf(stderr, "Error: " fmt "\n", __VA_ARGS__); exit(1); } while(0)

typedef struct { double sum; double c; } KahanSum;
...
static void rng_init(Rng *r, int extra_seed) {
    r->state = (unsigned long long)time(NULL)
...
int8_t *read_binary_data(const char *filename, size_t num_freq, size_t *out_size) {
    ...
    if (!f) FATALF("Cannot open file: %s", filename);
    fseek(f, 0, SEEK_END);
    long file_size = ftell(f);
    fseek(f, 0, SEEK_SET);
    if (file_size < 0)                     FATAL("ftell failed");
    if ((size_t)file_size % num_freq != 0) FATAL("File size not multiple of num_freq");
    int8_t *data = (int8_t *)malloc((size_t)file_size);
    if (!data) FATAL("malloc failed in read_binary_data");
...
void write_binary_data(const char *filename, const int8_t *data, size_t data_size,
    ...
    if (!f) FATALF("Cannot open file for writing: %s", filename);
    //if (data_size % nsamples != 0 || data_size % nchans != 0) FATALF("Data dimensions mismatch: %s", filename);
    if (fwrite(data, 1, data_size, f) != data_size) FATAL("Error writing file");
    fclose(f);
}
...
void float_to_int8(float *in, int8_t *out, size_t N, float outmean, float outstd) {
    ...
    if (!t_mean || !t_sq) FATAL("malloc failed float_to_int8");
...
void skf_filter(float *data, float thresig, size_t nsamples, size_t nchans) {
    if (nsamples == 0 || nchans == 0) FATAL("Empty data in skf_filter");
    ...
    for (size_t j = 0; j < nchans; ++j) {
        ...
        chmean1[j] = m1; chmean2[j] = m2;
        chmean3[j] = m3; chmean4[j] = m4;
        chcorr[j]  = corr;
    }
    ...
    #pragma omp parallel for schedule(static)
    for (size_t j = 0; j < nchans; ++j) {
        chmean1[j] /= nsamples; chmean2[j] /= nsamples;
        chmean3[j] /= nsamples; chmean4[j] /= nsamples;
        chcorr[j]  /= (nsamples - 1);
    ...
    double corr_q1, corr_q3, corr_R;
    {
        ...
        qsort(tmp, nchans, sizeof(float), cmp_float_asc);  kurtosis_q1 = tmp[q_idx];
        qsort(tmp, nchans, sizeof(float), cmp_float_desc); kurtosis_q3 = tmp[q_idx];
        kurtosis_R = kurtosis_q3 - kurtosis_q1;
        memcpy(tmp, chskewness, nchans * sizeof(float));
        qsort(tmp, nchans, sizeof(float), cmp_float_asc);  skewness_q1 = tmp[q_idx];
        qsort(tmp, nchans, sizeof(float), cmp_float_desc); skewness_q3 = tmp[q_idx];
        skewness_R = skewness_q3 - skewness_q1;
        free(tmp);
    }
    {
        double *tmp = (double *)malloc(nchans * sizeof(double));
        for (size_t j = 0; j < nchans; ++j) tmp[j] = chcorr[j];
        qsort(tmp, nchans, sizeof(double), cmp_double_asc);  corr_q1 = tmp[q_idx];
        qsort(tmp, nchans, sizeof(double), cmp_double_desc); corr_q3 = tmp[q_idx];
    ...
    if (!weights) FATAL("malloc failed skf step4");
    ...
    if (thresig >= 0) {
        ...
        #pragma omp parallel for schedule(static)
        for (size_t j = 0; j < nchans; ++j) weights[j] = 1;
    }
    ...
    free(chmean1); free(chmean2); free(chmean3); free(chmean4); free(chcorr);
    free(chmean); free(chstd); free(chskewness); free(chkurtosis);
    free(weights);
}
...
void patch_filter(float *data, size_t nsamples, size_t nchans, int filltype) {
    ...
    if (!mask) FATAL("malloc failed patch_filter mask");
    ...
    long kill_count = 0;
    for (size_t i = 0; i < nsamples; ++i) kill_count += mask[i];
    ...
    if (!chmean_patch || !chvar_patch) FATAL("malloc failed patch step2");
    ...
    if (count > 0) {
        for (size_t j = 0; j < nchans; ++j) {
            ...
            chvar_patch[j]   = chvar_patch[j] / count
                               - chmean_patch[j] * chmean_patch[j];
        }
    }
    ...
    if (filltype == 0) { /* mean fill */
        ...
        if (!ch_std) FATAL("malloc failed patch rand std");
        for (size_t j = 0; j < nchans; ++j)
...
void equalization(float *data, size_t nsamples, size_t nchans,
    ...
    if (!acc_mean || !acc_sq) FATAL("malloc failed equalization");
    ...
    if (!dmean||!dsq||!cmean_c||!csq_c) FATAL("malloc failed equalization merge");
    ...
    for (int t = 0; t < actual_threads; ++t) {
        ...
        for (size_t j = 0; j < nchans; ++j) {
            ...
            double y, tmp;
            y = lmean[j] - cmean_c[j]; tmp = dmean[j] + y;
            cmean_c[j] = (tmp - dmean[j]) - y; dmean[j] = tmp;
            /* Kahan add for dsq[j] */
            y = lsq[j] - csq_c[j]; tmp = dsq[j] + y;
            csq_c[j] = (tmp - dsq[j]) - y; dsq[j] = tmp;
        }
    }
    free(acc_mean); free(acc_sq); free(cmean_c); free(csq_c);
 ...
 * ====================================================================== */
typedef struct { float *arr; size_t size, cap; int is_max; } Heap;

static void heap_init(Heap *h, size_t cap, int is_max) {
    h->arr = (float *)malloc(cap * sizeof(float));
    h->size = 0; h->cap = cap; h->is_max = is_max;
}
static void heap_free_h(Heap *h) { free(h->arr); h->size = 0; }
...
static void heap_push(Heap *h, float v) {
    ...
    while (i > 0) {
        ...
        if (heap_cmp(h, h->arr[i], h->arr[p])) {
            float t = h->arr[i]; h->arr[i] = h->arr[p]; h->arr[p] = t; i = p;
        } else break;
    }
}
...
static void  heap_pop(Heap *h) {
    ...
    for (;;) {
        ...
        if (l < h->size && heap_cmp(h, h->arr[l], h->arr[best])) best=l;
        if (r < h->size && heap_cmp(h, h->arr[r], h->arr[best])) best=r;
        if (best==i) break;
        float t=h->arr[i]; h->arr[i]=h->arr[best]; h->arr[best]=t; i=best;
    }
}
    ...
    for (size_t i = 0; i < h->size; ++i) {
        if (h->arr[i] == v) {
            ...
            for (;;) {
                ...
                if (r<h->size&&heap_cmp(h,h->arr[r],h->arr[best])) best=r;
                if (best==j) break;
                float t=h->arr[j]; h->arr[j]=h->arr[best]; h->arr[best]=t; j=best;
            }
            return;
        }
    }
...
static void  heap_rebalance(Heap *low, Heap *high) {
    if      (low->size > high->size+1) { heap_push(high,heap_top(low)); heap_pop(low);  }
    else if (high->size > low->size+1) { heap_push(low, heap_top(high)); heap_pop(high); }
}
static float heap_median(const Heap *low, const Heap *high) {
    if (low->size  > high->size) return heap_top(low);
    if (high->size > low->size)  return heap_top(high);
    return (heap_top(low) + heap_top(high)) / 2.0f;
}

static void sliding_median_float(const float *data, float *out, long size, int w) {
    if (w > (int)size) w = (int)size;
    Heap low, high;
    ...
    for (int i=1; i<b && i<(int)size; ++i) {
        if (data[i]>=median) heap_push(&high,data[i]); else heap_push(&low,data[i]);
        heap_rebalance(&low,&high); median=heap_median(&low,&high);
    }
    for (int i=0; i<w/2+1 && b<(int)size; ++i) {
        if (data[b]>=median) heap_push(&high,data[b]); else heap_push(&low,data[b]);
        heap_rebalance(&low,&high); median=heap_median(&low,&high);
        out[i]=median; ++a; ++b;
    }
    for (int i=w/2+1; i<(int)size-(w-1)/2; ++i) {
        if (data[b]>=median) heap_push(&high,data[b]); else heap_push(&low,data[b]);
        int fl=0;
        for (size_t k=0;k<low.size;++k) if(low.arr[k]==data[a]){fl=1;break;}
        if (fl) heap_remove(&low,data[a]); else heap_remove(&high,data[a]);
        heap_rebalance(&low,&high); median=heap_median(&low,&high);
        out[i]=median; ++a; ++b;
    }
    ...
    heap_free_h(&low); heap_free_h(&high);
}
...
static void sliding_median_double(const double *data, double *out, long size, int w) {
    ...
    if (!fi||!fo) FATAL("malloc failed sliding_median_double");
    for (long i=0;i<size;++i) fi[i]=(float)data[i];
    sliding_median_float(fi,fo,size,w);
    for (long i=0;i<size;++i) out[i]=fo[i];
    free(fi); free(fo);
}
...
void baseline_filter(float *data, size_t nsamples, size_t nchans,
    ...
    if (!s) FATAL("malloc failed baseline s");
    ...
    if (!t_xe||!t_xs||!t_se||!t_ss) FATAL("malloc failed baseline regression");
    ...
    if (!xe||!xs||!cxe||!cxs) FATAL("malloc failed baseline merge");
    ...
    for (int t = 0; t < actual_threads; ++t) {
        ...
        for (size_t j = 0; j < nchans; ++j) {
            double y, tmp;
            y=lxe[j]-cxe[j]; tmp=xe[j]+y; cxe[j]=(tmp-xe[j])-y; xe[j]=tmp;
            y=lxs[j]-cxs[j]; tmp=xs[j]+y; cxs[j]=(tmp-xs[j])-y; xs[j]=tmp;
        }
        /* Kahan merge for se, ss */
        { double y=t_se[t]-cse; double tmp=se+y; cse=(tmp-se)-y; se=tmp; }
        { double y=t_ss[t]-css; double tmp=ss+y; css=(tmp-ss)-y; ss=tmp; }
    }
    free(t_xe); free(t_xs); free(t_se); free(t_ss);
    free(cxe); free(cxs);
    ...
    if (!alpha||!beta) FATAL("malloc failed baseline coeffs");
    ...
    free(s); free(xe); free(xs); free(alpha); free(beta);
}
...
static void build_output_path(const char *in_file, char *out_file, size_t out_len) {
    ...
    if (dir_len >= out_len) FATAL("Path too long");
    const char *dot  = NULL;
...
static void process_block(int8_t *block_ptr, size_t block_N,
    ...
    if (!fd||!cmean||!cstd) FATAL("malloc failed in process_block");
